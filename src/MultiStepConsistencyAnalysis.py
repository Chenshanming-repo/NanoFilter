...
def filter_by_consistency(consistency_dict_ctg, n_minimum_covered_reads, n_min_variant_position, consistency_threshold, vcf_fn, filtered_keys = None, filter_not_enough_position=False, filter_snp=True, use_avg_consistency=True, only_cal_with_snp=False, quality=15):
        ...
        for key1 in consistency_dict:
            ...
            if len(consistency_list) >= n_min_variant_position:
                ...
                if filter_not_enough_position:
                    if float(clair3_vcf_records_dict[key1].qual) < quality:
                        filtered_keys.add(key1)
...
def consistency_analysis_step_1(consistency_dict, n_minimum_covered_reads, n_min_variant_position, consistency_threshold, vcf_fn, filter_snp=True, quality=15):
    return filter_by_consistency(consistency_dict, n_minimum_covered_reads, n_min_variant_position, consistency_threshold, vcf_fn, filter_not_enough_position=False,filter_snp=filter_snp, use_avg_consistency=False, quality=quality)
def consistency_analysis_step_2(consistency_dict, n_minimum_covered_reads, n_min_variant_position, consistency_threshold, vcf_fn, filtered_keys, filter_snp=True, quality=15):
    return filter_by_consistency(consistency_dict, n_minimum_covered_reads, n_min_variant_position, consistency_threshold, vcf_fn, filtered_keys=filtered_keys,filter_not_enough_position=True,filter_snp=filter_snp, use_avg_consistency=True, quality=quality)
def consistency_analysis_step_3(consistency_dict, n_minimum_covered_reads, n_min_variant_position, consistency_threshold, vcf_fn, filtered_keys, filter_snp=True, quality=15):
    return filter_by_consistency(consistency_dict, n_minimum_covered_reads, n_min_variant_position, consistency_threshold, vcf_fn, filtered_keys=filtered_keys, filter_not_enough_position=True,filter_snp=filter_snp, use_avg_consistency=True, quality=quality)
...
def filter_main(VCF_FN, FRAGMENT_FN, OUTPUT_VCF, OUT_DIR, filter_parameters, FILTER_SNP=True, ctg=None):
    ...
    MIN_CONSISTENCY_STEP1 = float(filter_parameters[0])

    N_MIN_COVERED_READS_STEP2 = 10
    N_MIN_VARIANT_POSITION_STEP2 = 5
    MIN_CONSISTENCY_STEP2 = float(filter_parameters[1])

    N_MIN_COVERED_READS_STEP3 = 10
    N_MIN_VARIANT_POSITION_STEP3 = 5
    MIN_CONSISTENCY_STEP3 = float(filter_parameters[2])

    QUAL_THRESHOLD = int(float(filter_parameters[3]))
    ...
    # logger.info("============STEP1=============")
    filtered_keys_step1, consistency_info_list_step1 = consistency_analysis_step_1(consistency_res_dict, N_MIN_COVERED_READS_STEP1, N_MIN_VARIANT_POSITION_STEP1, MIN_CONSISTENCY_STEP1, VCF_FN, filter_snp=FILTER_SNP, quality=QUAL_THRESHOLD)
    ...
    # logger.info("============STEP2=============")
    filtered_keys_step2, consistency_info_list_step2 = consistency_analysis_step_2(consistency_res_dict, N_MIN_COVERED_READS_STEP2, N_MIN_VARIANT_POSITION_STEP2, MIN_CONSISTENCY_STEP2, VCF_FN, filtered_keys_step1, filter_snp=FILTER_SNP, quality=QUAL_THRESHOLD)
    ...
    # logger.info("============STEP3=============")
    filtered_keys_step3, consistency_info_list_step3 = consistency_analysis_step_3(consistency_res_dict, N_MIN_COVERED_READS_STEP3, N_MIN_VARIANT_POSITION_STEP3, MIN_CONSISTENCY_STEP3, VCF_FN, filtered_keys_step2, filter_snp=FILTER_SNP, quality=QUAL_THRESHOLD)
