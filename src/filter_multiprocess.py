...
from MultiStepConsistencyAnalysis import filter_main
...
def run_hapcut2_contigs(ctg, ref, bam, vcf, out_dir, only_snvs, rtype, only_extract_hairs):
    ...
    hapcut2_log_out = os.path.abspath(os.path.join(out_dir, "hapcut2.log"))
    hapcut2_log_err = os.path.abspath(os.path.join(out_dir, "hapcut2.err"))
    ...
    extract_hairs_cmd = [EXTRACT_HAIRS, "--indels", str(bool_to_int(not only_snvs)), "--ont", "1",
                         "--bam", os.path.abspath(bam), "--VCF", vcf_ctg,
                         "--out", os.path.abspath(FRAGMENT_FILE),
                         "--ref", os.path.abspath(ref),
                         "--region", ctg]

    with open(hapcut2_log_out, "w") as hapcut2_out, open(hapcut2_log_err, "w") as hapcut2_err:
        try:
            print(f"[CMD] {' '.join(extract_hairs_cmd)}")
            res = subprocess.check_call(extract_hairs_cmd, stdout=hapcut2_out, stderr=hapcut2_err)
        except subprocess.CalledProcessError as exc:
            print('returncode:', exc.returncode)
            print("cmd:", " ".join(exc.cmd))
            print(f"ERROR LOG: {hapcut2_log_err}")
...
def run_filter_multiprocess(ref, bam, vcf, new_vcf, out_dir, only_snvs, rtype, threads_num, FILTER_SNP, filter_parameters):
    ...
    p = Pool(threads_num)
    for ctg in pysam.VariantFile(vcf + ".gz").header.contigs:
        p.apply_async(filter_main, args=(f"{out_dir}/{ctg}/{os.path.split(vcf)[1]}.{ctg}", os.path.abspath(os.path.join(out_dir, ctg, "fragment_file")), f"{out_dir}/{ctg}/{os.path.split(vcf)[1]}.{ctg}.filtered", os.path.abspath(os.path.join(out_dir, ctg)), filter_parameters, FILTER_SNP, ctg), error_callback=on_error)
    p.close()
    p.join()
